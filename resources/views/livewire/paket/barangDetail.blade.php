...
new class extends Component {
    ...
    {
        dd('Submitting MOVE for:', $selectedRows);
    }
}; ?>
...
<div>
    ...
    <div class="relative my-6 bg-white border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-2xl gap-4"
        x-data="{
            selectedRows: @entangle('selectedRows').live,
            ...
            isSelected(id) {
                return this.selectedRows.some(r => r.id === id);
            }
        }">
                    ...
                    @foreach ($this->dataItems as $item)
                        <tr class="hover:bg-neutral-100 border-b border-neutral-300"
                            ...
                            <td class="py-4 px-2 text-center">
                                <input type="checkbox" @change="toggleRow({{ json_encode($item) }})"
        ...
        <!-- Action Button -->
        <div class="flex justify-end mt-4" x-show="selectedRows.length > 0" x-transition>
            <flux:modal.trigger name="action-modal">
                <flux:button variant="primary">
                    Action Select Data (<span x-text="selectedRows.length"></span>)
                </flux:button>
            </flux:modal.trigger>
        </div>
    ...
    <flux:modal name="action-modal" class="w-full max-w-6xl">
        <div class="space-y-6" x-data="{
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <template x-for="item in selectedRows" :key="item.id">
            ...
            <!-- Select Action Type -->
            <div class="space-y-2">
                <flux:label>Pilih Jenis Aksi</flux:label>
                <select x-model="actionType"
            ...
            <div x-show="actionType === 'SHIPPING'" x-cloak x-transition:enter="transition ease-out duration-300"
                ...
                class="mt-4 border rounded-lg p-4 bg-blue-50 shadow-inner relative">
            ...
            <div x-show="actionType === 'MOVE'" x-cloak x-transition:enter="transition ease-out duration-300"
                ...
                x-transition:leave="transition ease-in duration-200 absolute inset-0"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                class="mt-4 border rounded-lg p-4 bg-green-50 shadow-inner relative">
                <flux:heading size="md" class="mb-3 text-green-600">Form MOVE</flux:heading>

                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div>
                        <flux:label>Lokasi Asal</flux:label>
                        <flux:input placeholder="Lokasi awal barang" />
                    </div>
                    <div>
                        <flux:label>Lokasi Tujuan</flux:label>
                        <flux:input placeholder="Lokasi tujuan barang" />
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-3">
                        ...
                        <textarea class="border rounded-lg w-full p-2" placeholder="Keterangan perpindahan..."></textarea>
                    </div>
                </div>
            </div>
                ...
                <flux:button variant="primary" class="px-6 py-2"
                    x-on:click="
                        if (actionType === 'SHIPPING') {
                            $wire.call('submitShipping', selectedRows);
                        } else if (actionType === 'MOVE') {
                            $wire.call('submitMove', selectedRows);
                        } else {
